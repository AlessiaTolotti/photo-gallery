...
// import DriveImage from './DriveImage'; // Commentiamo temporaneamente finché non risolviamo le API
...
export default function PhotoGallery() {
  ...
  const syncWithFolder = async () => {
    ...
    try {
      ...
      try {
        console.log('💾 Tentativo cartella locale...');
        const response = await fetch('/api/photos/watch');
        const data = await response.json();
        
        console.log('📂 Risposta watch:', data);
  ...
  useEffect(() => {
    ...
    // RIMOSSO: Sincronizzazione automatica ogni 5 secondi che causava caricamento continuo
          ...
          photos.map((photo, index) => (
            ...
            >
              <div className="photo-wrapper">
                {photo.driveData ? (
                  // Placeholder per DriveImage finché non risolviamo
